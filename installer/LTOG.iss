...
; {app}\winltfs, its ltfs.conf shipped as-is with bare-name plugin paths).
...
; ltfs.conf ships as-is: its plugin paths are bare DLL names, resolved by ltfs.exe
; next to itself, so it needs no per-install rewrite. *.pdb symbols aren't needed.
...
{ Install the prerequisites. Runs once (DepDone guard), from either the
  dependency page (interactive) or ssPostInstall (silent). }
...
begin
  ...
  { WinFsp (installs when absent, upgrades a driver older than the bundled DLL). }
